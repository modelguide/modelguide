 ...
 *   bun run demos/seed-medusa/test-checkout-flow.ts [--dry-run]
 */

import { createSeedClients } from "./client";
...
async function run() {
  ...
  const { products } = await store<{
    products: { id: string; title: string; variants: { id: string; title: string }[] }[];
  ...
  const { cart: cartAfterAdd } = await store<{
    cart: { id: string; items: { id: string; title: string; quantity: number; unit_price: number }[] };
  ...
  console.log(`  Shipping address:   ${cartState.shipping_address ? "set" : "none"}`);
  console.log(`  Shipping methods:   ${cartState.shipping_methods?.length ?? 0}`);
  console.log(`  Payment collection: ${cartState.payment_collection ? "yes" : "none"}`);
  ...
  console.log(`      Shipping methods: ${preComplete.shipping_methods?.length ?? 0}`);
  console.log(`      Payment collection: ${preComplete.payment_collection?.id ?? "none"}`);
  ...
  const { order: fetchedOrder } = await store<{
    order: { id: string; display_id: number; status: string; total: number; currency_code: string };
  }>(`/store/orders/${order.id}`);

  console.log(`  Order #${fetchedOrder.display_id}`);
  console.log(`  Status:   ${fetchedOrder.status}`);
  console.log(`  Total:    ${fetchedOrder.total} ${fetchedOrder.currency_code}`);
