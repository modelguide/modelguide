...
class AssistantTranscriptProcessor(FrameProcessor):
    ...
    async def process_frame(self, frame, direction):
        await super().process_frame(frame, direction)
        # TextFrame contains chunks of text being sent to TTS
        from pipecat.frames.frames import TextFrame

        if isinstance(frame, TextFrame) and frame.text:
            self._current_sentence += frame.text
        # When we see an LLM full response end, flush the accumulated text
...
async def main():
    async with aiohttp.ClientSession() as http_session:
        ...
        session_id = await mg_client.create_session()
