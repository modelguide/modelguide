...
describe("registerCoreTools", () => {
  ...
  test("registers all expected tool names", () => {
    ...
    expect(names).toContain("core_set_customer");
  });
  ...
  test("core_set_customer schema has session_id, name, email, phone", () => {
    const { server, registeredTools } = createMockServer();
    registerCoreTools(server as never, "org-1", "agent-1");

    const tool = registeredTools.find((t) => t.name === "core_set_customer")!;
    expect(tool.schema.session_id).toBeDefined();
    expect(tool.schema.name).toBeDefined();
    expect(tool.schema.email).toBeDefined();
    expect(tool.schema.phone).toBeDefined();
  });
